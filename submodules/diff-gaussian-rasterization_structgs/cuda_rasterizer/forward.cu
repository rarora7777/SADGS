...
__device__ float4 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	// 1. Calculate the determinants (Area of the 2D Gaussian)
	float det_0 = max(1e-6f, cov[0][0] * cov[1][1] - cov[0][1] * cov[0][1]);
	float kernel_size=0.1f;
	// 2. Add the filter (kernel_size) to the diagonal
	cov[0][0] += kernel_size;
	cov[1][1] += kernel_size;

	// 3. Calculate new determinant after dilation
	float det_1 = max(1e-6f, cov[0][0] * cov[1][1] - cov[0][1] * cov[0][1]);

	// 4. Calculate the compensation coefficient
	...
	float* normals)
{
	...
	cov2Ds[idx * 7 + 0] = cov.x;
	cov2Ds[idx * 7 + 1] = cov.y;
	cov2Ds[idx * 7 + 2] = cov.z;
	cov2Ds[idx * 7 + 3] = ndc2Pix(p_proj.x, W);
	cov2Ds[idx * 7 + 4] = ndc2Pix(p_proj.y, H);
	...
	float4 con_o = { conic.x, conic.y, conic.z, opacities[idx] * cov.w };
	...
	// Compute normal from rotation quaternion (z-axis of rotated frame)
	if (normals != nullptr)
	{
		glm::vec4 q = rotations[idx];
		float r = q.x, x = q.y, y = q.z, z = q.w;
		// z-axis of rotation matrix: third column
		normals[idx * 3 + 0] = 2.f * (x * z + r * y);
		normals[idx * 3 + 1] = 2.f * (y * z - r * x);
		normals[idx * 3 + 2] = 1.f - 2.f * (x * x + y * y);
	}
}
	...
	float* __restrict__ out_normal)
{
	...
	// Extra accumulators for depth and normal
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Accumulate depth and normal if compute_extra is enabled
			if (compute_extra)
			{
				float w = alpha * T;
				accum_depth += depths[collected_id[j]] * w;
				accum_normal[0] += normals[collected_id[j] * 3 + 0] * w;
				accum_normal[1] += normals[collected_id[j] * 3 + 1] * w;
				accum_normal[2] += normals[collected_id[j] * 3 + 2] * w;
			}
	...
	if (inside)
	{
		...
		// Write extra outputs if enabled
		if (compute_extra)
		{
			out_depth[pix_id] = accum_depth;
			out_opacity[pix_id] = 1.0f - T;
			// Normalize accumulated normal
			float norm_len = sqrtf(accum_normal[0] * accum_normal[0] + 
			                       accum_normal[1] * accum_normal[1] + 
			                       accum_normal[2] * accum_normal[2]);
			if (norm_len > 1e-6f) {
				out_normal[0 * H * W + pix_id] = accum_normal[0] / norm_len;
				out_normal[1 * H * W + pix_id] = accum_normal[1] / norm_len;
				out_normal[2 * H * W + pix_id] = accum_normal[2] / norm_len;
			} else {
